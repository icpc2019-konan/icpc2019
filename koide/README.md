...
上の手順を一般化する．カウンタ i を k周余分に押す場合に，カウンタ0～iのすべてを目標値にする最小押下回数 d[i][k]は，$i>0$に対して，  

$ d[i][k] = \min\{ dd[i][k][l] \ \text{ for }l = 0, 1, ..., m-1\} $  
$ dd[i][k][l] = \left\{ \begin{array}{ll} d[i-1][l] & \text{if } c[i][k] < c[i-1][l] \\ d[i-1][l] + c[i][k] - c[i-1][l] & \text{otherwise} \end{array}\right. $
